...
-   **Think out loud:**
    ...
    Comment: The country and entity variables.
    ...
    Comment: We would use left_join for this.
...
Comment: I think that the highest is Asia, second will probably be Africa and the lowest, if it included in this data set, would be Antarctica or if not it would likely be Oceania.
...
ggplot(population_summary, aes()) +
  ...
  geom_segment(aes(y= continent, x = 0, xend = total_pop)) +
...
Comment: Orient the graph so that contients are on the x-axis, add some clearer labels, re order the continents in order of highest to lowest population.
